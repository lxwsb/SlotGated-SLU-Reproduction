...
# ... existing code from train.py第22行开始拷贝即可 ...
...
def createModel(input_data, input_size, sequence_length, slot_size, intent_size, layer_size = 128, isTraining = True):
    ...
    if isTraining == True:
        cell_fw = tf.compat.v1.nn.rnn_cell.DropoutWrapper(cell_fw, input_keep_prob=0.5, output_keep_prob=0.5)
        cell_bw = tf.compat.v1.nn.rnn_cell.DropoutWrapper(cell_bw, input_keep_prob=0.5, output_keep_prob=0.5)
...
with tf.Session() as sess:
    ...
    epoch_loss_history = []
    valid_f1_history = []
    valid_intent_history = []  # 新增intent accuracy历史曲线
    ...
    while True:
        ...
        if data_processor.end == 1:
            ...
            epoch_mean_loss_for_record = loss/num_loss
            logging.info('Loss: ' + str(epoch_mean_loss_for_record))
            num_loss = 0
            loss = 0.0
            ...
            # record metrics for plotting (x-axis uses iteration step at epoch end)
            steps_history.append(int(step))
            epoch_loss_history.append(float(epoch_mean_loss_for_record))
            valid_f1_history.append(float(epoch_valid_slot))
            valid_intent_history.append(float(epoch_valid_intent))  # 记录intent准确率
    ...
    # After training loop: write metrics and plot if possible
    # Ensure model_path exists
    try:
        if not os.path.isdir(arg.model_path):
            os.makedirs(arg.model_path)
    except Exception:
        pass

    # Write CSV of metrics
    metrics_csv = os.path.join(arg.model_path, 'training_metrics.csv')
    try:
        with open(metrics_csv, 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(['step', 'epoch_mean_loss', 'valid_slot_f1', 'valid_intent_acc'])  # 增加intent acc
            # align lengths
            max_len = max(len(steps_history), len(epoch_loss_history), len(valid_f1_history))
            for i in range(max_len):
                s = steps_history[i] if i < len(steps_history) else ''
                l = epoch_loss_history[i] if i < len(epoch_loss_history) else ''
                f1v = valid_f1_history[i] if i < len(valid_f1_history) else ''
                int_acc = valid_intent_history[i] if i < len(valid_intent_history) else ''
                writer.writerow([s, l, f1v, int_acc])
    except Exception:
        pass

    # Plot curves
    if _HAS_MPL and len(steps_history) > 0:
        try:
            plt.figure(figsize= (8,5))
            ax1 = plt.gca()
            # plot loss if available (may contain NaN)
            if len(epoch_loss_history) == len(steps_history):
                ax1.plot(steps_history, epoch_loss_history, '-o', label='Loss/损失值', color='#1f77b4', linewidth=1.5, markersize=3)
                ax1.set_ylabel('Loss/损失函数')
            ax1.set_xlabel('Iterations (global step)')

            ax2 = ax1.twinx()
            ax2.plot(steps_history, valid_f1_history, '-s', label='F1/插槽F1', color='#d62728', linewidth=1.5, markersize=3)
            ax2.plot(steps_history, valid_intent_history, '-^', label='Intent Acc/意图准确率', color='#2ca02c', linewidth=1.5, markersize=3)
            ...
            plt.title('Training Curves/训练曲线')
            plt.tight_layout()
            out_png = os.path.join(arg.model_path, 'training_curves.png')
